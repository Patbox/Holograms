...
public class TextParserUtils {
    ...
    public static String removeEscaping(String string) {
        for (GeneralUtils.Pair<String, String> entry : UNESCAPED_CHARS) {
            try {
    ...
    public static GeneralUtils.TextLengthPair recursiveParsing(String input, Map<String, TextParser.TextFormatterHandler> handlers, String endAt) {
        if (input.isEmpty()) {
                ...
                if (lines.length > 1) {
                    ...
                    if (action == HoverEvent.Action.SHOW_TEXT) {
                        ...
                        out.text().setStyle(Style.EMPTY.withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM,
                                new HoverEvent.ItemStackContent(ItemStack.fromTag(StringNbtReader.parse(removeEscaping(cleanArgument(lines[1])))))
                        )));
                    }
                }
            } catch (Exception e) {
